...
const WeatherForecast = ({ forecast, units }) => {
  const dailyForecast = forecast?.list.reduce((acc, item) => {
    ...
    if (!acc[date]) {
      // 해당 날짜의 첫 번째 항목만 저장한다. - 3시간 단위로 분리된 하루 예보 중 가장 앞에 있는 항복을 지정
      acc[date] = item;
    }
  ...
  const datilyItems = Object.values(dailyForecast).slice(0, 5);
  // console.log(datilyItems);
  return (
